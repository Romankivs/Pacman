...
float* monster1 = new float[3] {10, 8, 1.0}; //coordinates and direction of first monster
float* monster2 = new float[3] {13, 1, 2.0}; //coordinates and direction of second monster
float* monster3 = new float[3] {4, 6, 3.0}; //coordinates and direction of third monster
...
double monstersSpeedMultiplier = 0.15;

int randomLocationX;
int randomLocationY;
int randomLocationCounter;
...
void updateMonster(float* monster, SearchAlgorithm alg = SearchAlgorithm::AStar, bool guard = false) {
	bool needToRecaltulateDirection = false;
	auto t = (abs(int(monster[0]) - monster[0]));
	auto gt = (abs(int(monster[1]) - monster[1]));
	bool onecond = t <= 2 / squareSize * monstersSpeedMultiplier;
	bool secondcond = gt <= 2 / squareSize * monstersSpeedMultiplier;
	if (onecond && secondcond)
	{
 		needToRecaltulateDirection = true;
	}

	//move him acording to its direction until he hits an obstacle
	if (needToRecaltulateDirection)
	{
		int pacmanX = floor(1.5 + xIncrement);
		int pacmanY = floor(1.5 + yIncrement);

		if (guard)
		{
			if (pacmanX < 7) // guard right half of the map
			{
				if (randomLocationCounter < 5)
				{
					randomLocationCounter++;
				}
				else
				{
					do
					{
						std::uniform_int_distribution<std::mt19937::result_type> dist(7, 13);
						randomLocationX = dist(rng);
						randomLocationY = dist(rng);
						randomLocationCounter = 0;
					} while (bitmap.at(pacmanX).at(pacmanY));
				}
				pacmanX = randomLocationX;
				pacmanY = randomLocationY;
			}
		}

		if (alg == SearchAlgorithm::AStar)
		{
			monster[2] = AStar::aStarSearch(bitmap, { (int)floor(monster[0]), (int)floor(monster[1]), 0, 0 }, { pacmanX, pacmanY, 0, 0 });
		}
		else if (alg == SearchAlgorithm::Greedy)
		{
			monster[2] = Greedy::greedyBestFirstSearch(bitmap, { (int)floor(monster[0]), (int)floor(monster[1]), 0 }, { pacmanX, pacmanY, 0 });
		}
	}
	switch ((int)monster[2]) {
	case 1:
		monster[0] -= 2 / squareSize * monstersSpeedMultiplier;
		break;
	case 2:
		monster[0] += 2 / squareSize * monstersSpeedMultiplier;
		break;
	case 3:
		monster[1] -= 2 / squareSize * monstersSpeedMultiplier;
		break;
	case 4:
 		monster[1] += 2 / squareSize * monstersSpeedMultiplier;
		break;
...
void resetGame() {
	...
	yIncrement = 0;
	rotation = 0;
	monster1 = new float[3] {10, 8, 1.0};
	monster2 = new float[3] {13, 1, 2.0};
	monster3 = new float[3] {4, 6, 3.0};
	points = 0;
	for (int i = 0; i < 256; i++) {
		keyStates[i] = false;
	}
...
void keyOperations() {
	...
	if (keyStates['a']) {
		...
		if (!bitmap.at(x1Quadrant).at((int)y / squareSize)) {
			xIncrement -= 2 / squareSize * speedMultiplier;
			rotation = 2;
		}
	}
	else if (keyStates['d']) {
		...
		if (!bitmap.at((int)x / squareSize).at(y1Quadrant)) {
			yIncrement -= 2 / squareSize * speedMultiplier;
			rotation = 3;
		}
	}
	else if (keyStates['s']) {
	...
	if (keyStates['1'])
	{
		monstersSpeedMultiplier = 0.1;
	}
	else if (keyStates['2'])
	{
		monstersSpeedMultiplier = 0.15;
	}
	else if (keyStates['3'])
	{
		monstersSpeedMultiplier = 0.23;
	}
}
...
void resultsDisplay() {
	if (points == 106) {
		...
		message = (char*)"CONGRATULATIONS, YOU WON! ";
		glColor3f(1, 1, 1);
		...
		while (*message)
			glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++);
		message = (char*)"To start or restart the game, press the space key.";
...
void welcomeScreen() {
	...
	char* message = (char*)"To control Pacman use A to go right, D to go left, W to go up and S to go down.";
	glRasterPos2f(50, 300);
	while (*message)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *message++);
	message = (char*)"To select difficulty press 1 - Easy, 2 - Medium, 3 - Hard. Default is Medium.";
...
void display() {
	...
	if (replay) {
		if (!over) {
			...
			updateMonster(monster1, SearchAlgorithm::AStar);
			updateMonster(monster2, SearchAlgorithm::AStar, true);
			updateMonster(monster3, SearchAlgorithm::Greedy);
